<template>
  <v-container>
        ...
        <v-list>
          <v-list-item v-for="(product) in order.products" :key="product.id" class="elevation-2 mb-2 rounded-lg">
            ...
            <v-list-item-content>
              ...
              <v-list-item-subtitle class="font-weight-bold primary--text mt-1">$ {{product.product.price}}
              </v-list-item-subtitle>
            </v-list-item-content>
            <v-list-item-action v-if="order.status=='Pending'">>
              <v-input>
    ...
    <v-card width="100%" class="card-resume rounded-t-xl">
      ...
      <v-card-actions class="pl-6 pr-6" v-if="order.status=='Pending'">
  ...
  export default {
    ...
    created() {
      this.$store.dispatch('titlePages/setTitlePage', {
        text: `Orden n#: ${this.$route.params.id_order}`,
        color: 'secondary',
      })
      this.getOrder()
    },
