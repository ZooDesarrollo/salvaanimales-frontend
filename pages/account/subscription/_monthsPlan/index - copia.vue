<template>
  <v-container>
    <v-card class="rounded-lg">
      ...
      <v-card-text>
        <form action="/charge" method="post" id="payment-form" ref="form">
          <v-row>
            <v-col class="col-12">
              <v-text-field flat filled class="rounded-lg" hide-details required placeholder="Nombre completo"
                type="text">
              </v-text-field>
            </v-col>
            <v-col class="col-12">
              <v-text-field flat filled class="rounded-lg" hide-details required placeholder="Documento" type="text">
              </v-text-field>
            </v-col>
            <v-col class="col-12">
      ...
      <v-card-actions>
        <v-btn block color="secondary" large class="font-weight-black rounded-lg" @click="submitForm()">SUBSCRIBIRME
          AHORA</v-btn>
      </v-card-actions>
    </v-card>
  </v-container>
    ...
    methods: {
      ...
      submitForm() {
        if(!this.fieldsFilled.cvv && !this.fieldsFilled.expiration && !this.fieldsFilled.card)return 
          this.dlocal.createToken(this.cvvField, {
