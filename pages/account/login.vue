  ...
  <v-container d-flex align-center class="">
    <v-row no-gutters class="fill-height align-content-space-around d-flex">
      ...
      <v-col class="col-12 pa-4">
        <v-text-field prepend-inner-icon="mdi-email" required label="Email" type="email" class="font-weight-medium"
        ...
        <v-text-field required prepend-inner-icon="mdi-key-variant" label="Password" :type="show1 ? 'text' : 'password'"
          :append-icon="show1 ? 'mdi-eye' : 'mdi-eye-off'" @click:append="show1 = !show1" class="font-weight-medium"
          v-model="profile.password">
        </v-text-field>
        <v-btn x-large style="width:100%" depressed class="white--text font-weight-bold rounded-lg" color="primary"
          @click="loginUser()">INICIAR SESION</v-btn>
        <div class="d-flex">
          <v-spacer></v-spacer>
          <span @click="showNewPassword = true" class="grey--text mt-3 font-weight-medium">¿Olvidaste tu contraseña?</span>
        </div>
        <!--
    ...
    <v-dialog v-model="showNewPassword" max-width="500px" transition="dialog-transition" class="text-center">
      <v-card>
        ...
        <v-card-text>
          ...
          <v-text-field name="email" label="Tu email" v-model="email" color="primary"></v-text-field>
        </v-card-text>
        <v-card-actions>
    ...
    <v-dialog v-model="showResetPass" max-width="500px" transition="dialog-transition" class="text-center">
      <v-card>
        ...
        <v-card-text>
          ...
          <v-text-field name="codigo" label="Codigo" v-model="codigo"></v-text-field>
          <v-text-field name="contraseña" label="Contraseña" v-model="resetPass"></v-text-field>
          <v-text-field name="contraseña_confirm" label="Confirmacion de contraseña" v-model="resetPassConf">
          </v-text-field>
        </v-card-text>
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn color="primary" class="white--text" @click="resetPassword">Cambiar contrase;a</v-btn>
          <v-spacer></v-spacer>
        </v-card-actions>
      </v-card>
...
<script>
  ...
  export default {
    ...
    computed: {
      snackBarColor() {
        return (this.snackBarMessage == 'Credenciales incorrectas' || this.snackBarMessage ==
          'Contraseñas no coinciden') ? 'red' : 'green'
      }
    },
    methods: {
      newPassword() {
